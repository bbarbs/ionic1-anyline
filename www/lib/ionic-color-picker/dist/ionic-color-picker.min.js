            ...
            return {
                ...
                link: function (scope, iElement, iAttrs) {
                    var pristines = {
                        ...
                        modelMode = iAttrs.modelMode || "hex",
                        strokeColors = iAttrs.availableStrokeColors ? iAttrs.availableStrokeColors.split(",") : ["#ffffff", "#000000"],
                        colorNames = iAttrs.namedColors ? scope.$eval(iAttrs.namedColors) : tinycolor.names,
                        colorNamesInverted = {};
                    angular.forEach(colorNames, function (v, k) {
                        var v2 = 0 == v.indexOf("#") ? v : "#" + v;
                        v2 = tinycolor(v2).toHexString(), colorNamesInverted[v2] = k
                    });
                    var tColor, txt;
                    iAttrs.$observe("colorBox", function (nv) {
                        if (!nv) return iElement.css("background-color", pristines.backgroundColor), iElement.css("color", pristines.color), void iElement.css("border-color", pristines.borderColor);
                        if ("name" == modelMode) nv = colorNames[nv], nv = 0 == nv.indexOf("#") ? nv : "#" + nv, tColor = tinycolor(nv);
                        else if ("hex" == modelMode) nv = 0 == nv.indexOf("#") ? nv : "#" + nv, tColor = tinycolor(nv);
                        else try {
                            nv = scope.$eval(nv), tColor = tinycolor.fromRatio(nv)
                        } catch (err) {
                            return
                        }
                        txt = tinycolor.mostReadable(tColor, strokeColors).toHexString(), "true" !== iAttrs.colorBoxNoBorder && iElement.css("border-color", txt), "true" !== iAttrs.colorBoxNoText && iElement.css("color", txt), iElement.css("background-color", tColor.toHexString())
                    })
                }
            }
            ...
            return {
                ...
                link: function (scope, iElement, iAttrs, ngModelController) {
                    var colorMode = iAttrs.colorMode || "rgb",
                        customColors = void 0 !== iAttrs.namedColors,
                        colorNames = iAttrs.namedColors ? scope.$eval(iAttrs.namedColors) : tinycolor.names,
                        colorNamesInverted = {},
                        shortListBreak = iAttrs.shortListBreak ? parseInt(iAttrs.shortListBreak) : 10,
                        modelMode = iAttrs.modelMode || "hex";
                    angular.forEach(colorNames, function (v, k) {
                        var v2 = 0 == v.indexOf("#") ? v : "#" + v;
                        v2 = tinycolor(v2).toHexString(), colorNamesInverted[v2] = k
                        ...
                        hideReset: "true" !== iAttrs.hideReset ? !1 : !0,
                    ...
                    for (var key in scope.ui.namedColors) {
                        ...
                        k = 0 == k.indexOf("#") ? k : "#" + k, scope.ui.namedColorsAsList.push([key, k])
                    }
                    ...
                    var initial = ngModelController.$viewValue,
                        getScopeColor = function (c) {
                            return "rgb" == colorMode ? c.toRgb() : "hsl" == colorMode ? c.toHsl() : "hsv" == colorMode ? c.toHsv() : void 0
                        };
                    initial && (customColors ? c = tinycolor(colorNames[initial]) : c = tinycolor(initial), scope.internalColors[colorMode] = getScopeColor(c)), scope.unsetColor = function () {
                        $timeout(function () {
                            ngModelController.$setViewValue(""), scope.modal.hide(), scope.showList = !1
                        })
                    };
                    var setColorTo = function (col, fun) {
                            ngModelController.$setViewValue(col[fun]())
                        },
                        setColorToName = function (col) {
                            var theCol = colorNamesInverted[col.toHexString()];
                            return theCol ? void ngModelController.$setViewValue(colorNamesInverted[col.toHexString()]) : void console.error("cannot set color", col)
                        };
                    scope.setColor = function (col) {
                        switch (col = "name" == colorMode ? tinycolor(scope.ui.namedColors[col]) : tinycolor(getCurrentColorModel()), modelMode) {
                        case "name":
                            setColorToName(col);
                            break;
                        case "rgb":
                        ...
                        scope.modal.hide(), scope.showList = !1
                    }, scope.closeModal = function () {
                        scope.modal.hide(), scope.showList = !1;
                        var v = ngModelController.$viewValue;
                        v && (v = tinycolor(v), scope.internalColors[colorMode] = getScopeColor(v))
                    }, scope.toggleSelectMode = function () {
                        scope.ui.selectMode = "sliders" == scope.ui.selectMode ? "form" : "sliders"
                    };
                    var tplName = "modal-template-" + scope.ui.mode + ".html";
                    scope.modal = $ionicModal.fromTemplate(colorPickerTemplates[tplName], {
                        scope: scope
                    }), scope.$on("$destroy", function () {
                        scope.modal.remove()
                    }), iElement.on("click", function () {
                        shortList ? (scope.showList = !0, scope.modal.show()) : scope.modal.show().then(function () {
                            scope.showList = !0
                        })
                    });
                    var getCurrentColorModel = function () {
                        if ("hex" != colorMode) {
                            var out = {};
                            return angular.forEach(scope.internalColors[colorMode], function (v, k) {
                                out[k] = parseFloat(v)
                            }), out
                        }
                    ...
                    scope.$watch(function () {
                        ...
                        nv && (scope.ui.sample = tinycolor(nv).toHexString())
                    }, !0)
                }
            }
        }])
